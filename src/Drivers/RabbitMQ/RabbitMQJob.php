...
final class RabbitMQJob extends Job implements JobContract
{
    ...
    public function release($delay = 0): void
    {
        ...
        // Reject the message without requeue (we'll push a new one)
        $this->rabbitmq->reject($this->envelope, $this->queue, false);

        // Push a new message with delay (delay is converted to milliseconds in options)
        $options = $delay > 0 ? ['delay' => $delay * 1000] : [];
        $this->rabbitmq->pushRaw($this->preparePayloadForRelease(), $this->queue, $options);
    }
    ...
    public function delete(): void
    {
        ...
        $this->rabbitmq->ack($this->envelope, $this->queue);
    }
    ...
    {
        // Acknowledge the message first to remove it from the queue
        $this->rabbitmq->ack($this->envelope, $this->queue);
    ...
    public function getJobId(): string
    {
        ...
        return $payload['uuid'] ?? $this->envelope->getMessageId() ?? '';
    }
    ...
    /**
     * Get the underlying AMQP envelope.
     */
